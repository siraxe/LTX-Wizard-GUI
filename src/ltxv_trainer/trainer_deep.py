...
import argparse # Import argparse for DeepSpeed arguments
...
class LtxvDeepSpeedTrainer: # Renamed class to LtxvDeepSpeedTrainer
    def __init__(self, trainer_config: LtxvTrainerConfig) -> None:
        self._config = trainer_config
        self._console = Console()
        self._print_config(trainer_config)
        
        # Initialize Accelerator for general utilities (e.g., is_main_process)
        ...
        self._load_models() # Load raw models
        self._compile_transformer() # Compile raw transformer
        self._setup_block_swap() # Apply block swap to raw transformer
        self._collect_trainable_params() # Setup LoRA or unfreeze for full training on raw transformer
        self._init_optimizer() # Initialize raw optimizer and scheduler
        self._init_dataloader() # Initialize raw dataloader

        # Now initialize DeepSpeed with the prepared components
        self._setup_deepspeed()
        
        self._load_checkpoint() # Load checkpoint using DeepSpeed's mechanism
        
        self._dataset = None # Dataset is initialized in _init_dataloader
        self._global_step = -1
    ...
    def _setup_block_swap(self) -> None:
        ...
        # The transformer is not yet wrapped by DeepSpeed at this point, so we use it directly.
        ...
        # DeepSpeed will manage device placement, but for block swap, we need to know the target.
        # Assuming the primary device is CUDA if available, otherwise CPU.
    ...
    def train(  # noqa: PLR0912, PLR0915
        ...
        """
        # DeepSpeed manages device, so we get it from model_engine
        device = self._model_engine.device
        cfg = self._config
        ...
        self._model_engine.train() # Use model_engine.train()
        self._global_step = 0
        ...
        with Live(Panel(Group(train_progress, sample_progress)), refresh_per_second=2):
            ...
            for step, batch in enumerate(self._data_loader): # Iterate directly over DeepSpeed's data_loader
                ...
                # with self._accelerator.accumulate(self._transformer): # Removed accelerator.accumulate
                is_optimization_step = (step + 1) % cfg.optimization.gradient_accumulation_steps == 0
                if is_optimization_step:
                    self._global_step += 1

                loss = self._training_step(batch)
                self._model_engine.backward(loss) # DeepSpeed backward
                ...
                self._model_engine.step() # DeepSpeed step
                # self._optimizer.zero_grad() # DeepSpeed handles zero_grad
                ...
                self._accelerator.wait_for_everyone() # Still useful for general synchronization
            ...
            self._accelerator.end_training() # Keep accelerator end_training for cleanup
    ...
    def _training_step(self, batch: dict[str, dict[str, Tensor]]) -> Tensor:
        ...
        # Use self._model_engine for forward pass
        model_pred = self._model_engine( # Changed to self._model_engine
    ...
    def _load_models(self) -> None:
        ...
        # DeepSpeed handles device placement, so no need for accelerator.prepare here
        # prepare = self._accelerator.prepare # Removed accelerator.prepare
        ...
        # Prepare components with accelerator (only for non-DeepSpeed managed models)
        self._scheduler = components.scheduler
        self._tokenizer = components.tokenizer
        self._text_encoder = components.text_encoder # Not prepared by accelerator
        self._vae = components.vae # Not prepared by accelerator
        ...
        self._transformer = transformer # Not prepared by accelerator initially
        ...
        # Enable gradient checkpointing if requested (DeepSpeed will handle this)
    ...
    def _collect_trainable_params(self) -> None:
        ...
        # DeepSpeed will manage trainable parameters, so self._trainable_params is not needed here.
        # It will be set by deepspeed.initialize if needed for gradient clipping, etc.
        self._trainable_params = [p for p in self._transformer.parameters() if p.requires_grad]
        logger.debug(f"Trainable params count (before DeepSpeed wrap): {sum(p.numel() for p in self._trainable_params):,}")
    ...
    def _load_checkpoint(self) -> None:
        ...
        # DeepSpeed will handle loading its own checkpoints
        # transformer = self._accelerator.unwrap_model(self._transformer) # Removed accelerator.unwrap_model

        logger.info(f"📥 Loading checkpoint from {checkpoint_path}")

        if self._config.model.training_mode == "full":
            # DeepSpeed's load_checkpoint expects a directory and global_step
            # The checkpoint_path from _find_checkpoint for full mode is already the directory
            try:
                global_step = int(checkpoint_path.name.replace("global_step", ""))
            except (IndexError, ValueError):
                raise ValueError(f"Could not extract global step from DeepSpeed checkpoint path: {checkpoint_path}")
            
            load_dir = checkpoint_path
            self._model_engine.load_checkpoint(load_dir, tag=f"global_step{global_step}")
            logger.info(f"DeepSpeed checkpoint loaded from {load_dir} at step {global_step}")
        else:  # LoRA mode
            # Load LoRA weights into the DeepSpeed-wrapped model's module
            ...
            _, unexpected_keys = self._model_engine.module.load_state_dict(state_dict, strict=False)
            if unexpected_keys:
                raise ValueError(f"Failed to load some LoRA weights: {unexpected_keys}")
            logger.info(f"LoRA weights loaded from {checkpoint_path}")
    ...
    def _init_dataloader(self) -> None:
        ...
        # DeepSpeed will prepare the data loader
        # noinspection PyTypeChecker
        # data_loader = self._accelerator.prepare(data_loader) # Removed accelerator.prepare
    ...
    def _init_optimizer(self) -> None:
        ...
        if opt_cfg.optimizer_type == "adamw":
            optimizer = AdamW(self._transformer.parameters(), lr=lr) # Pass transformer parameters
        elif opt_cfg.optimizer_type == "adamw8bit":
            # noinspection PyUnresolvedReferences
            from bitsandbytes.optim import AdamW8bit  # type: ignore

            optimizer = AdamW8bit(self._transformer.parameters(), lr=lr) # Pass transformer parameters
        else:
        ...
        # DeepSpeed will prepare optimizer and scheduler
        # noinspection PyTypeChecker
        # self._optimizer, self._lr_scheduler = self._accelerator.prepare(optimizer, lr_scheduler) # Removed accelerator.prepare
    ...
    def _setup_deepspeed_accelerator(self) -> None:
        """Initialize DeepSpeed and Accelerator."""
        # Initialize Accelerator for general utilities (e.g., is_main_process)
        # DeepSpeed will handle distributed training setup
        self._accelerator = Accelerator(
            mixed_precision=self._config.acceleration.mixed_precision_mode,
            gradient_accumulation_steps=self._config.optimization.gradient_accumulation_steps,
        )
        ...
        # DeepSpeed config can be passed via a JSON file or a dictionary
        # For now, we'll assume a deepspeed_config.json path is provided in trainer_config
        if not self._config.deepspeed.config_path:
            raise ValueError("DeepSpeed configuration file path is required for DeepSpeed training.")

        deepspeed_config_path = self._config.deepspeed.config_path
        ...
        # DeepSpeed handles model, optimizer, and data loader wrapping
        # These will be initialized in _init_optimizer and _init_dataloader
        # and then passed to deepspeed.initialize
        
        # DeepSpeed requires a dummy parser for its arguments
        parser = argparse.ArgumentParser(description="DeepSpeed training")
        parser = deepspeed.add_config_arguments(parser)
        # Add any other arguments DeepSpeed might need, e.g., local_rank
        # For now, assume local_rank is handled by accelerate or environment
        
        # Parse known arguments to avoid issues with unknown args from other configs
        args, _ = parser.parse_known_args()
        args.deepspeed_config = deepspeed_config_path # Set the config path

        # Initialize DeepSpeed
        # The model, optimizer, and lr_scheduler will be passed later in _init_optimizer
        # For now, we just set up the distributed environment
        deepspeed.init_distributed()
        logger.info(f"DeepSpeed initialized with config: {deepspeed_config_path}")
    ...
    def _sample_videos(self, progress: Progress) -> list[Path] | None:
        ...
        # Unwrap the model for inference
        pipeline_transformer = self._model_engine.module # Use .module to get the unwrapped model
        pipeline_transformer.eval() # Set to eval mode for inference
        ...
        if use_images:
            ...
            pipeline = LTXImageToVideoPipeline(
                ...
                transformer=pipeline_transformer, # Use unwrapped transformer
            )
        else:
    ...
    def _save_checkpoint(self) -> Path:
        ...
        # DeepSpeed handles saving the model state
        # unwrapped_model = self._accelerator.unwrap_model(self._transformer) # Removed accelerator.unwrap_model

        if self._config.model.training_mode == "full":
            # DeepSpeed saves the full model, optimizer, and LR scheduler state
            self._model_engine.save_checkpoint(save_dir, self._global_step, client_state={"step": self._global_step})
            logger.info(f"💾 DeepSpeed checkpoint for step {self._global_step} saved in {rel_saved_weights_path}")
        elif self._config.model.training_mode == "lora":
            # For LoRA, we still need to extract and save PEFT state dict
            state_dict = get_peft_model_state_dict(self._model_engine.module) # Get state dict from unwrapped model
        ...
        if 0 < self._config.checkpoints.keep_last_n < len(self._checkpoint_paths):
            ...
            for old_checkpoint in checkpoints_to_remove:
                if old_checkpoint.exists():
                    if self._config.model.training_mode == "full":
                        # For full model, old_checkpoint is a directory (e.g., global_step00001)
                        if old_checkpoint.is_dir():
                            import shutil
                            shutil.rmtree(old_checkpoint)
                            logger.debug(f"Removed old DeepSpeed checkpoint directory: {old_checkpoint}")
                    else: # LoRA mode, old_checkpoint is a .safetensors file
                        old_checkpoint.unlink()
                        logger.debug(f"Removed old LoRA checkpoint: {old_checkpoint}")

...
def save_loss_plot(
    ...
    try:
        ...
        logger.error(f"Error saving loss plot for step {global_step}: {e}")
        # Create an empty error file
        error_file_path = output_dir / f"loss_plot_step_{global_step:06d}_error.txt"
        with open(error_file_path, "w") as f:
            f.write(f"Error saving loss plot: {e}")